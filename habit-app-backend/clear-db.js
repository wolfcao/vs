...
const clearDatabase = async () => {
  try {
    ...
    await sequelize.transaction(async (t) => {
      ...
      const [timeResult] = await sequelize.query('DELETE FROM time_modification_requests', { transaction: t });
      console.log(`已删除 ${timeResult.affectedRows || 0} 条时间修改请求记录`);
      
      console.log('正在清空 team_members 表...');
      const [memberResult] = await sequelize.query('DELETE FROM team_members', { transaction: t });
      console.log(`已删除 ${memberResult.affectedRows || 0} 条团队成员记录`);
      
      console.log('正在清空 active_habits 表...');
      const [activeResult] = await sequelize.query('DELETE FROM active_habits', { transaction: t });
      console.log(`已删除 ${activeResult.affectedRows || 0} 条活跃习惯记录`);
      
      // 保留习惯定义和用户数据
      // await sequelize.query('DELETE FROM habits', { transaction: t });
