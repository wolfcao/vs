...
import { ArrowRight, Flame, CheckCircle2, Clock, AlertCircle } from "lucide-react";

const Dashboard: React.FC = () => {
  const { myActiveHabits, user } = useHabit();
  const navigate = useNavigate();
  const [activeTab, setActiveTab] = useState<"active" | "pending" | "created" | "rejected">('active');

  // Filter habits by status and member approval status
  const activeHabits = myActiveHabits.filter(habit => {
    // Active status and user is approved member
    const userMember = habit.members?.find(member => member.userId === user?.id);
    return habit.status === 'active' && userMember?.approvalStatus === 1;
  });
  
  const pendingHabits = myActiveHabits.filter(habit => {
    // Pending status or user is pending approval
    const userMember = habit.members?.find(member => member.userId === user?.id);
    return habit.status === 'pending' || userMember?.approvalStatus === 0;
  });
  
  // Filter habits where user is rejected
  const rejectedHabits = myActiveHabits.filter(habit => {
    const userMember = habit.members?.find(member => member.userId === user?.id);
    return userMember?.approvalStatus === -1;
  });
  ...
  const displayedHabits = activeTab === 'active' ? activeHabits : 
                         ...
                         activeTab === 'rejected' ? rejectedHabits :
                         createdHabits;
      ...
      <div className="border-b border-border mb-6">
        <div className="flex space-x-8 overflow-x-auto">
          <button
            ...
            className={`py-3 px-1 border-b-2 font-medium text-lg transition-colors whitespace-nowrap ${activeTab === 'active' ? 'border-primary text-primary' : 'border-transparent text-textSecondary hover:text-text'}`}
          >
            已加入
          </button>
          <button
            onClick={() => setActiveTab('pending')}
            className={`py-3 px-1 border-b-2 font-medium text-lg transition-colors whitespace-nowrap ${activeTab === 'pending' ? 'border-primary text-primary' : 'border-transparent text-textSecondary hover:text-text'}`}
          >
            申请中
          </button>
          <button
            onClick={() => setActiveTab('rejected')}
            className={`py-3 px-1 border-b-2 font-medium text-lg transition-colors whitespace-nowrap ${activeTab === 'rejected' ? 'border-primary text-primary' : 'border-transparent text-textSecondary hover:text-text'}`}
          >
            已拒绝
          </button>
          <button
            onClick={() => setActiveTab('created')}
            className={`py-3 px-1 border-b-2 font-medium text-lg transition-colors whitespace-nowrap ${activeTab === 'created' ? 'border-primary text-primary' : 'border-transparent text-textSecondary hover:text-text'}`}
          >
            我创建的
          </button>
        </div>
        ...
        {displayedHabits.length === 0 ? (
          ...
          <div className="grid gap-6">
            {displayedHabits.map((active) => {
              ...
              // Check user's member status in this habit
              const userMember = active.members?.find(member => member.userId === user?.id);
              const isRejected = userMember?.approvalStatus === -1;
                ...
                >
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center space-x-2">
                        ...
                        {active.status === 'pending' && (
                          <span className="ghibli-tag ghibli-tag-default text-xs">
                            待审核
                          </span>
                        )}
                        {isRejected && (
                          <span className="ghibli-tag bg-red-500 text-white text-xs">
                            已拒绝
                          </span>
                        )}
                      </div>
                    ...
                    <div className="flex -space-x-2">
                      {active.members?.filter(m => m.approvalStatus === 1).map((m, i) => (
                        <img
                          key={i}
                          src={m.avatar}
                          alt={m.name}
